...
indata.n_id_S=[7,2,3,1,6,1,6,2];%225*ones(1,100); % kept fixed-interface normal modes for each group of components [1 2 1 36 33 9 27 27 29 32 25 24 30 7 20 1 1 1 1 69 37 1] [4     9     7     2     5     2     6     6]

% this is used if method=1
r=2*ones(1,100); % multiplication constant used to define the target frequency 2.5, 2.6,2
...
indata.eigf.group.method_store=0; % 0 or 1

% this is used if method_store=0
indata.n_id_S_store=50*ones(1,100); % large values

% this is used if method_store=1
indata.eigf.group.target_store=30*2000*ones(1,100); % large cutoff frequency
...
indata.n_IR=31; % kept interface modes for all interfaces (global reduction)

% this is used if method=0 and local reduction is selected
indata.n_IR_l=30*ones(1,100); % kept interface modes for each interface (local reduction)

% this is used if method=1
v=13*ones(1,100); % multiplication constant used to define the target frequency
...
indata.eigf.interface.step_store=10*ones(1,100); % large values
indata.eigf.interface.init_store=10*ones(1,100); % large values
% ---
...
indata.E=35*10^9*ones(22,1); % Young's modulus [Pa] for each group of components
indata.nu=.2*ones(22,1); % Poisson's ratio
indata.rho=2500*ones(22,1); % density [kg/m^3]
% ---
