function [matdata,indata]=model_updating_local(dofdata,indata)
...
if ~isfile('submatdata_interp_local.mat')
    submatdata_interp=matfile('submatdata_interp_local.mat','Writable',true);
...
submatdata_interp=load('submatdata_interp_local.mat');

% check that n_IR_l is correct. If not, change n_IR_l.
[~,n_IR_l_correct]=cellfun(@size,submatdata_interp.YPSILON_I_theta_0);
if ~isequal(n_IR_l_correct,indata.n_IR_l)
    warning(['Input vector n_IR_l does not correspond to the number of kept interface modes for each interface.'...
        ' Will change the input vector to correspond properly.']);
    indata.n_IR_l=n_IR_l_correct;
    indata.n_IRL=sum(indata.n_IR_l);
    indata.n_DIL=indata.n_id+indata.n_IRL;
end

% % to ensure that the number of kept interface modes for each interface
% % corresponds to the input file (submatdata_interp). Else, there is error
% % when calculating interface modes in a sample point which does not belong
% % in the convex hull of the existing support points.
% indata.eigf.interface.method=0;

if ~isfile('submatdata_6.mat') 
    submatdata=matfile('submatdata_6.mat','Writable',true);
...
submatdata=load('submatdata_6.mat');
...
tic
matdata=matassem_para_local([],submatdata,mats_interp_k,dofdata,indata); 
toc
